...
(async () => {
    try {
        ...
        `);
        // Safely alter existing tables if migrating from an older version
...
// --- API STATE (STRICT DATA ISOLATION) ---
app.get('/api/state', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (req.user.role === 'admin') {
            ...
            // Strict isolation: Hotels ONLY see data tied to their owner_email
            // Explicit CAST added to fix PostgreSQL Type Mismatch Error 42883
            myTrades = (await db.query(tQ + " WHERE seller = $1 OR buyer = $1", [req.user.email])).rows;
            myBookings = (await db.query(bQ + " WHERE CAST(token_id AS VARCHAR) IN (SELECT CAST(token_id AS VARCHAR) FROM room_inventory WHERE owner_email = $1)", [req.user.email])).rows;
            myInventory = (await db.query(iQ + " WHERE owner_email = $1", [req.user.email])).rows;
        } else {
            // TA and Supplier sees their interactions and global inventory to map names
            myTrades = (await db.query(tQ + " WHERE seller = $1 OR buyer = $1", [req.user.email])).rows;
...
app.post('/admin/create-inventory', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // REPLICATES CREATOR EMAIL FOR ISOLATION
        await db.query(
...
app.post("/admin/reset", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const defaultUsers = [
            ...
            { email: "admin@supplier.com", password: "12345", role: "supplier" }
        ];
